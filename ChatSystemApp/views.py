...
def chatPage(request,follower_id,following_id):
    ...
    if request.method == 'POST':
        ...
        if len(msg)!=0:
            ...
            new_msg.save()
            return redirect('chatPage',follower_id,following_id)
    ...
    if FollowList.objects.filter(follower=follower1,following=following1).exists() and FollowList.objects.filter(follower=follower2,following=following2).exists():
        ...
        requested = False
        follow_back = False
    context = {'user':user,'follow':follow,'requested':requested,'follow_back':follow_back,'follower':follower1,'friends_count':friends_count}
...
def roomchatPage(request,room_id):
    ...
    if request.method == 'POST':
        ...
        if len(msg) != 0:
            ...
            new_msg.save()
            return redirect('roomchatPage',room_id)
...
def deleteroomMessage(request,msg_id):
    ...
    msg.delete()
    return redirect('roomchatPage',msg.room_name.id)